...
func (r *IntegrationRepository) UpdateInstanceLastWebhook(ctx context.Context, instanceID string, headers, body json.RawMessage, lastAt time.Time) error {
	...
	log.Info().
		...
		Msg("🔧 Repository: saving webhook data")
	...
	if err != nil {
		log.Error().Err(err).Str("instance_id", instanceID).Msg("Repository: failed to update last webhook")
	} else {
		log.Info().Str("instance_id", instanceID).Msg("Repository: webhook saved successfully")
	}
	return err
}
