...
func (h *Handler) GetLastWebhook(c echo.Context) error {
	...
	headersStr := prettyHeaders.String()
	bodyStr := prettyBody.String()

	return c.HTML(http.StatusOK, fmt.Sprintf(`
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" id="webhook-modal">
            <div class="relative top-20 mx-auto p-5 border w-[1400px] shadow-lg rounded-md bg-white" style="max-width: 95%%;">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-medium">Последний запрос (%s)</h3>
                    <button onclick="document.getElementById('webhook-modal').remove()" class="text-gray-500 hover:text-gray-700 text-xl">✕</button>
                </div>
                
                <div class="mb-4">
                    <div class="flex justify-between items-center mb-2">
                        <h4 class="font-medium">Заголовки:</h4>
                        <div class="space-x-2">
                            <button onclick="copyToClipboard(document.getElementById('headers-content').textContent)" 
                                    class="text-xs bg-gray-200 hover:bg-gray-300 px-2 py-1 rounded">
                                Копировать
                            </button>
                            <button onclick="downloadJson(document.getElementById('headers-content').textContent, 'webhook-headers.json')" 
                                    class="text-xs bg-green-600 text-white hover:bg-green-700 px-2 py-1 rounded">
                                Скачать
                            </button>
                        </div>
                    </div>
                    <pre class="bg-gray-50 p-3 rounded text-sm overflow-auto max-h-80 font-mono border" id="headers-content">%s</pre>
                </div>
                
                <div>
                    <div class="flex justify-between items-center mb-2">
                        <h4 class="font-medium">Тело запроса:</h4>
                        <div class="space-x-2">
                            <button onclick="copyToClipboard(document.getElementById('body-content').textContent)" 
                                    class="text-xs bg-gray-200 hover:bg-gray-300 px-2 py-1 rounded">
                                Копировать
                            </button>
                            <button onclick="downloadJson(document.getElementById('body-content').textContent, 'webhook-body.json')" 
                                    class="text-xs bg-green-600 text-white hover:bg-green-700 px-2 py-1 rounded">
                                Скачать
                            </button>
                        </div>
                    </div>
                    <pre class="bg-gray-50 p-3 rounded text-sm overflow-auto font-mono border" style="max-height: 600px;" id="body-content">%s</pre>
                </div>
